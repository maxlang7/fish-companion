...
class Ask:
    ...
    def __repr__(self):
        # part AI
        asker_name = (
            self.asker.name if isinstance(self.asker, Player) else str(self.asker)
        )
        asked_name = (
            self.asked.name if isinstance(self.asked, Player) else str(self.asked)
        )
...
class Analyzer:
    ...
    def getAllLegalAsks(gameState, player, hand):
        ...
        potentialPlayers = [
            p for p in gameState.players if p.team != player.team and p.handSize > 0
        ]
...
class Listener:
    ...
    def parseText(self, text, current_asker):
        ...
        if s_score >= confidence_threshold:
            found_suit = s_match

        v_match, v_score = process.extractOne(text, self.card_values)
        if v_score >= confidence_threshold:
            found_value = v_match
        ...
        if (
            found_suit is not None
            and found_value is not None
            and found_player is not None
        ):
    ...
    def run_automated_test(app, test_key):
        ...
        if test_key in all_tests:
            ...
            for action in all_tests[test_key]:
                if action[0] == "seed":
                    TestManager.seed_card(
                        app.gameState, action[1], action[2], action[3]
                    )
                elif action[0] == "move":
                    TestManager.run_test_move(
...
def onAppStart(app):
    ...
    app.gameState = LiteratureGame(
        ["max", "alex", "ben"], ["jack", "kevin", "darren"], do_log=not is_testing
    )
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "test":
            for test_key in all_tests:
                app.gameState = LiteratureGame(
                    ["max", "alex", "ben"], ["jack", "kevin", "darren"], do_log=False
                )
...
def redrawAll(app):
    ...
    if app.isReplay:
        ...
        status_color = "orange"
        progress = (
            (app.replayIndex / len(app.replayMoves)) * 400 if app.replayMoves else 0
        )
        if progress > 0:
    ...
    drawLabel(
        f"Sets: {app.gameState.teams[0].setsTaken}", 105, 150, size=24, fill="blue"
    )

    drawRect(210, 100, 170, 80, fill="white", border="lightGrey")
    drawLabel("Team 2", 295, 120, size=16, bold=True)
    drawLabel(
        f"Sets: {app.gameState.teams[1].setsTaken}", 295, 150, size=24, fill="red"
    )
    ...
    if app.gameState.asks:
        ...
        drawLabel(
            "Waiting for first move...", 200, 300, size=14, italic=True, fill="grey"
        )
