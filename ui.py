...
# Overall UI entirely written with AI, designed by me and made lots of changes asking it
# Refactored some code myself
...
class Renderer:
    ...
    startupButtons = {
        "play":  (318, 702, 180, 58, "PLAY",       "Start digital game"),
        "short": (511, 702, 180, 58, "SHORT GAME",  "2 sets, quick game"),
        "watch": (704, 702, 180, 58, "WATCH",       "All-bot simulation"),
        "deal":  (897, 702, 180, 58, "DEAL CARDS",  "Physical game setup"),
    }
    ...
    def drawFish(app, x, y, size, color, direction):
        ...
        Renderer.label(
            app, "▶" if direction > 0 else "◀", tail_x, y, size=size, fill=color
        )
    ...
    def drawCardGrid(
        ...
        sortedCards = sorted(cards, key=cardSortKey)
        col = row = 0
        for card in sortedCards:
            Renderer.drawCard(
                app, card, startX + col * xGap, startY + row * yGap, cardW, cardH
            )
            col += 1
            if col == cardsPerRow:
                col = 0
    ...
    def drawStartupScreen(app):
        ...
        Renderer.label(
            app, "How to Play", 400, 283, size=22, bold=True, fill=colorDeepSea
        )
        rules = [
        ...
        Renderer.label(
            app, "Game Modes", 1000, 283, size=22, bold=True, fill=colorDeepSea
        )
        modeLines = [
            ("PLAY",       "Start a digital game. Non-human seats become bots."),
            ("SHORT GAME", "Play with just 2 sets (12 cards)."),
            ("WATCH",      "Sit back and watch 6 bots play."),
            ("DEAL CARDS", "Reveal each player's hand for a physical card game."),
        ]
        y = 326
        ...
        humanCount = app.startupHumanCount
        Renderer.label(
            app, "Human players:", W // 2, 622, size=17, bold=True, fill="white"
        )
        n = 0
        for cx, cy, r in Renderer.playerBubbles:
            ...
            Renderer.circle(
                ...
                border="gold" if selected else "white",
                borderWidth=2,
            )
            Renderer.label(
                app,
        ...
        summary = (
            f"{humanCount} {hWord} + {bots} {botWord}"
            if bots > 0
            else f"{humanCount} humans, no bots"
        )
        ...
        for key, (bx, by, bw, bh, blabel, bsub) in Renderer.startupButtons.items():
            ...
            Renderer.rect(
                app, bx, by, bw, bh, fill=bgColor, border=bdColor, borderWidth=2
            )
            Renderer.label(
                app,
                blabel,
            ...
            Renderer.label(
                app, bsub, bx + bw // 2, by + bh * 0.75, size=11, fill=subColor
            )

        if humanCount > 1:
            Renderer.label(
                app,
                "Use DEAL CARDS for multi-player games",
                W // 2,
                776,
                size=13,
                fill="gold",
            ...
            Renderer.label(
                app,
                "Type 1–6 to set player count  •  SPACE to play vs bots",
                W // 2,
                776,
                size=13,
                fill="white",
    ...
    def drawDealScreen(app):
        ...
        Renderer.label(
            app,
            f"Fish — Dealing Cards  ({min(pidx + 1, total)} of {total})",
            W // 2,
            29,
            size=26,
            bold=True,
        ...
        if not app.dealRevealed:
            ...
            Renderer.label(
                app,
                "Press SPACE to reveal your cards.",
                W // 2,
                607,
                size=18,
                fill=colorWaterMid,
            ...
            Renderer.label(
                app,
                f"{player.name}'s Hand  ({len(player.hand)} cards)",
                W // 2,
                91,
                size=22,
                bold=True,
            ...
            if pidx + 1 < total:
                ...
                Renderer.label(
                    app,
                    f"Done? Press SPACE to pass to {nxt}.",
                    W // 2,
                    696,
                    size=18,
                    fill=colorDeepSea,
                ...
                Renderer.label(
                    app,
                    "Done? Press SPACE to start the game.",
                    W // 2,
                    696,
                    size=18,
                    fill=colorDeepSea,
    ...
    def drawPlayerTable(app):
        ...
        Renderer.label(
            app,
            f"T1: {app.gameState.teams[0].setsTaken}",
            tableCx - 40,
            tableCy,
            size=18,
            bold=True,
        ...
        Renderer.label(
            app,
            f"T2: {app.gameState.teams[1].setsTaken}",
            tableCx + 40,
            tableCy,
            size=18,
            bold=True,
        ...
        for seat in range(6):
            ...
            if player.wantsTheTurn:
                Renderer.circle(
                    app, lx, ly, circleR + 13, fill=None, border="lime", borderWidth=3
                )
    ...
    def drawInputBox(app):
        ...
        if app.declareMode:
            Renderer.rect(app, 50, sy - 5, 500, 28, fill="darkViolet", border=None)
            if app.declarePhase == "select":
                Renderer.label(
                    app,
                    "DECLARE MODE — type set code (e.g. LH, EJ)",
                    tableCx,
                    sy + 9,
                    size=13,
                    bold=True,
                ...
                remaining = ", ".join(str(c) for c in app.declareRemaining)
                Renderer.label(
                    app,
                    f"Declaring {app.declareSet} — remaining: {remaining}",
                    tableCx,
                    sy + 9,
                    size=12,
                    bold=True,
        ...
        if app.illegalAskMessage:
            ...
            sy += 40
        elif not app.declareMode:
            Renderer.label(
                app,
                "Type shortcode  |  D = declare",
                tableCx,
                sy + 15,
                size=16,
                fill="white",
        ...
        if app.declareMode and app.declarePhase == "select":
            hints = ["Set", "Code"]
            nSlots = 2
        elif app.declareMode and app.declarePhase == "assign":
            hints = ["Who", "Val", "Suit"]
            nSlots = 3
        else:
        ...
        total = nSlots * slotW + (nSlots - 1) * gap
        sx0 = tableCx - total // 2
        cx = sx0 + slotW // 2
        for hint in hints:
        ...
        for i in range(nSlots):
            ...
            Renderer.rect(
                ...
                fill="white",
                opacity=90,
                border="violet" if app.declareMode else ("gold" if active else "white"),
                borderWidth=2,
            )
            if ch:
                Renderer.label(
    ...
    def drawUserHand(app):
        ...
        sortedHand = sorted(user.hand, key=cardSortKey)
        for i in range(len(sortedHand)):
            Renderer.drawCard(
                app, sortedHand[i], startX + i * xGap, startY, cardW, cardH
            )
    ...
    def drawInfoPanel(app):
        ...
        Renderer.label(
            app,
            "A=Ace  2–9  T=Ten  J=Jack  Q=Queen  K=King  X=Joker",
            x,
            y + 25,
            size=12,
            align="left",
        ...
        Renderer.label(
            app,
            "H=Hearts  D=Diamonds  C=Clubs  S=Spades  R=Red  B=Black",
            x,
            y + 45,
            size=12,
            align="left",
        ...
        Renderer.label(
            app,
            "Example: 2JH (Ask Player 2 for Jack of Hearts)",
            x,
            y + 65,
            size=12,
            italic=True,
    ...
    def drawAll(app):
        ...
        Renderer.label(
            app, "Fish: Ocean Edition", W // 2, 30, size=32, bold=True, fill="white"
        )
        ...
        Renderer.label(
            app, turnText, tableCx, 540, size=18, fill=colorDeepSea, bold=True
        )

        if app.gameState.asks:
            last = app.gameState.asks[-1]
            askX, askY = 1000, 560
            Renderer.rect(app, 720, 535, 630, 90, fill="black", opacity=50, border=None)
            Renderer.label(app, str(last), askX, askY, size=18, fill="white", bold=True)
            rc = "springGreen" if last.gotCard else "orangeRed"
            Renderer.label(
                app,
                "✓ SUCCESS" if last.gotCard else "✗ FAILED",
                askX,
                askY + 30,
                size=20,
                bold=True,
        ...
        Renderer.label(
            app,
            "R=Reset  Esc=Clear  Bksp=Delete  P=Want Turn  D=Declare",
            W // 2,
            H - 15,
            size=12,
            fill="white",
        ...
        if app.declareMessageTimer > 0:
            msg, success = app.declareMessage
            color = "darkGreen" if success else "crimson"
            Renderer.rect(
                app,
                W // 2 - 360,
                340,
            ...
            Renderer.label(app, msg, W // 2, 395, size=30, bold=True, fill="white")

        if app.gameState.winner:
            Renderer.rect(app, 0, 0, W, H, fill="black", opacity=60)
            if app.gameState.winner == "tied":
                Renderer.rect(
                    app,
                    W // 2 - 200,
                    H // 2 - 50,
                ...
                Renderer.label(
                    app, "IT'S A TIE!", W // 2, H // 2, size=40, bold=True, fill="white"
                )
            else:
                ...
                Renderer.label(
                    app, f"TEAM {teamNum} WINS!", W // 2, H // 2, size=40, bold=True
                )
